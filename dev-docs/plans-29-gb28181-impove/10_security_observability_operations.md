...
| ID | 控制 | 验收 |
| --- | --- | --- |
| SEC-01 | SIP Digest 使用 CSPRNG nonce、期限、qop/nc/cnonce、realm 和 replay cache | replay/expiry/bruteforce matrix |
...
| SEC-04 | SIP/RTP/PS/XML/framing 所有长度和集合都有 hard limit | fuzz/OOM guard |
| SEC-05 | signaling control plane 使用 mTLS、SAN/source identity 与证书轮换 | live rotation test |
...
- signaling：owner/lease/drain state、mutation outcome、stale generation、idempotency replay。

## 3. 事件与日志

结构化事件至少包括 resource ref、generation、state transition、reason code、compat rule、统计摘要和
correlation ID。endpoint、device ID 和 StreamKey 按安全策略哈希/截断；原始 SIP/SDP/XML/RTP 只允许
在受控测试环境抓取，不进入默认生产日志。
...
- local owner readiness：配置有效、port pool 可用、必要 module Running。
- signaling owner readiness：以上条件 + mTLS server + fixed contract + registry lease + facade/event/
  recovery 装配完成。
...
- 启动时输出不含 secret 的 resolved capability/profile/limits/control owner/contract revision。
...
- runbook 覆盖端口耗尽、RTP 无流、PS 无 PSM、SSRC 冲突、source change、RTCP timeout、双 owner、
