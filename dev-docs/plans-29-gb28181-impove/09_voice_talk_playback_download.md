...
独立匿名 socket/map。业务命令由 signaling 决定，media 只执行授权后的媒体绑定。
...
| TALK-06 | cancel/BYE/timeout/send failure 统一释放 task/socket/lease | leak tests |

Broadcast/MANSCDP 信令由 signaling 发送；其成功不代表 media ready。只有 RTP talk session 进入
Active 后才报告最终 applied outcome。

## 3. Playback 与 download

| ID | 实施 | 完成证据 |
| --- | --- | --- |
| PLAY-01 | command 显式携带 record source、start/end、Subject/SSRC 和目标 transport | contract tests |
...
- live、playback、download 使用显式 media purpose，生成对应 Subject、时间范围和 SSRC。
...
- signaling response loss 通过 idempotency/get query 找回同一 session，不重复启动 reader/worker。
