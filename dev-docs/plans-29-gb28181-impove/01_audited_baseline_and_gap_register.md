...
| ID | 差距 | 风险 | 目标章节 |
| --- | --- | --- | --- |
| GAP-API-01 | GB module 通过内部 HTTP/JSON 编排 RTP | 跨层/错误语义丢失风险已缓解；已迁移到 `RtpSessionApi` typed 调用 (P1 API-02) | 04、07 |
| GAP-ADM-01 | receiver 可能先分配 RTP 资源再鉴权 | Deny 后资源泄漏风险已缓解；`MediaAdmissionApi::authorize` 在 socket/port 分配前拒绝 (P1 ADM-01/02) | 07 |
...
| GAP-RTP-01 | RTP reorder、source binding/rebind、RTCP 生命周期（SR/RR/SDES/BYE/timeout）与非标准 PT 处理已由 RTP-CORE-02..04、RTCP-01..05、CODEC-RTP-03..04 实现；仍待真实 fixture 转 Supported | 丢包、串流、NAT 不稳风险已缓解，需互操作证据解除 | 05、06 |
...
| GB28181 module | CODE PASS | 已迁移到 `RtpSessionApi` typed 调用；旧内部 HTTP/JSON `call_rtp_service` 已移除 (P1 API-02/03/04、ADM-01/02) |
| PS/TS/ES 共享编解码 | CODE PASS | `cheetah-codec` 提供基础视图 |
| admission/fence/capacity | CODE PASS | admission、profile 白名单、capacity、idempotency 已接入 `RtpSessionApi` open/stop 路径 (ADM-01/02) |
| typed media API/RtpSessionApi | CODE PASS | `RtpSessionApi` 已定义 typed request/result、generation、resource ref、idempotency 与 admission gate；GB module 已迁移到 typed API (P1 API-01..04 / ADM-01/02) |
| 第三方控制接口 | CODE PASS | `RtpSessionApi` get/list roundtrip (EXT-06) 与 `SessionOpened`/`SessionClosed` 媒体事件 (EXT-07) 已合并；公开媒体接口 typed 查询与事件完成 |
| per-principal request rate limit | CODE PASS | `RtpMediaProvider` 在 open/update/stop 路径接入滑窗限流，配置 `request_rate_limit_per_minute`，集成测试覆盖 (SEC-01, PR #358) |
| media mutation tenant/owner/generation validation | CODE PASS | `RtpMediaProvider` 在 open/update/stop 入口校验 `MediaMutationContext` 的 `owner_epoch`、`target_media_node_instance_epoch`、`contract_version`；集成测试覆盖 (SEC-02) |
| secret/authorization redaction in logs | CODE PASS | `AuthCredentials` 与 `HttpRequest` 自定义 `Debug`，`authorization_header`、`deployment_token` 与 `Authorization`/`X-ZLM-Secret`/`Cookie` 头值在日志中脱敏为 `<redacted>`；单元测试覆盖 (SEC-06) |
| mTLS/SAN/source identity and certificate rotation | BLOCKED | mTLS 终止、SAN 提取与证书轮换由外部网络 adapter 负责；媒体面已提供 `AuthCredentials::mtls_identity` 与 source binding 策略，并新增 `docs/security/gb28181-rtp-mtls-source-identity.md` 定义接口契约；证书轮换/热重载需要 adapter 改造 (SEC-05) |
| RTP module session lifecycle metrics | CODE PASS | 新增 `RtpModuleMetrics`，在 open/stop/rollback/admit/rate-limit 路径计数，并写入引擎 `MetricsApi`；`rtp_sessions_active` gauge 与 orchestrator 实时会话数同步；集成测试 `rtp_module_metrics_record_open_close_and_active_gauge` 覆盖 (OBS-02) |
| UDP port pool and per-session allocation | CODE PASS | `RtpDriverConfig` 新增 `udp_port_pool`，`PortManager` 在端口 0 请求时从有界端口池分配并回收，driver 集成测试覆盖 (MOD-04) |
| RTP module file split and stop semantics | CODE PASS | 将 `module.rs` 拆分为 `http_service.rs`、`ingress.rs` 与 `module.rs`；`RtpHttpService` 负责 REST 路由与字段解析，`run_ingress_worker` 负责入站事件到引擎发布，`module.rs` 保留工厂、生命周期与拉流监督；单文件均低于 800 行 (MOD-05) |
| ABL/ZLM/SMS/JTT/Ehome2 media profile container mapping | CODE PASS | `MediaContainer` 新增 `Ehome2`/`Jtt1078`；`RtpMediaProvider` 校验 profile/container 一致性并映射 codec hint；集成测试覆盖 (COMPAT-01) |
| controlled resource/reconcile/event (MOD-02) | CODE PASS | `RtpSessionApi` 新增 `reconcile_sessions`；`RtpMediaProvider` 停止不在 `keep` 列表中的会话并触发 `SessionClosed` 事件；集成测试覆盖 (MOD-02) |
| media operations runbooks | CODE PASS | 新增 `docs/runbooks/gb28181-rtp-media-runbook.md`，覆盖端口耗尽、RTP 无流、PS 无 PSM、SSRC 冲突、source change、RTCP timeout、controller/registry outage 和 rollback incomplete；关联 `rtp_sessions_active`、`rtp_rollback_total` 等指标 (OPS-01) |
| release evidence draft | BLOCKED | 新增 `dev-docs/release_evidence/v0.1.0-draft-2026-07-24.md`，汇总 commit/Cargo.lock、fmt/clippy/test 结果、能力矩阵状态；secret scan、SBOM 生成、性能 benchmark、24h soak harness 已提供；真实 24h 长稳运行、真实设备互操作、SBOM 签名/attestation、外部 adapter mTLS 热轮换仍依赖外部环境与时间 (REL-01) |
