...
- [ ] 用 `cargo tree`/`cargo metadata` 生成实际依赖图，比对期望分层，标记任何反向或跨层依赖。
- [ ] `cheetah-codec` 不依赖 engine/module/HTTP/DB/具体 runtime（`AGENTS.md` §2）。
- [ ] `cheetah-sdk` 不反向依赖任何具体协议 module；HTTP module 契约不绑定 Axum/Tide/Actix。
- [ ] feature module 仅通过 `cheetah-sdk` + `cheetah-codec` 与系统交互，无“偷依赖”。
- [ ] `cheetah-runtime-api` / `cheetah-sdk` / `cheetah-engine` / `*-module` 公共接口不出现 `tokio::*`、
...
- [ ] 协议主路径命名为 `cheetah-<proto>-core/driver-tokio/module`，磁盘目录为 `core/`、`driver-tokio/`、`module/`。
- [ ] 属性测试为 `cheetah-<proto>-property-tests`（禁止 `pbt` 缩写）；绑定为 `-c-api`/`-wasm`。
- [ ] fuzz 为独立 cargo-fuzz workspace，默认不在根 workspace members 中。
- [ ] 全仓无 `plugin` 命名残留（类型名/模块名/目录名统一 `module`）。
...
- [ ] rtmp / rtsp / http-flv / hls / ts / fmp4 / mp4 / rtp / gb28181 / srt / webrtc 均具备三段式。
- [ ] `ts` 缺少 `testing/property-tests`（对比其他协议）→ 记录为测试覆盖缺口。
- [ ] 各 fuzz 目录是否游离于根 workspace（符合约定）。

### 1.4 架构文档一致性（P0 关键交付）

- [ ] `SystemArchitecture.md` 补齐 hls/ts/mp4/srt/webrtc 的 Reference Mapping 与 capability snapshot。
- [ ] README 的 feature 列表、端口、URL 与 `config.example.yaml`、`apps/cheetah-server` 默认 feature 一致。
...
- [ ] **统一媒体模型**：所有协议入口收敛为 `AVFrame + TrackInfo`；无协议自带私有 frame 模型。
- [ ] **三级时间线**（`time.rs`）：source / canonical / egress 边界正确；
      - source 时序仅作元数据，不得覆盖 canonical 顺序；
      - egress 时间戳修复不得回写 canonical 时间线。
- [ ] **AVFrame 语义**（`frame.rs`）：`pts/dts/duration` 同时携带 timebase ticks 与 `*_us`；
      `FrameFlags::KEY / DISCONTINUITY / B_FRAME` 语义正确；`pts < dts` 必须显式 `B_FRAME`。
- [ ] **归一化职责集中**：时间戳归一化、timebase 转换、DTS 生成、回绕、断流标记、Access Unit 拼装、
      参数集缓存/补发全部在 codec，不散落到协议（对照 §3 协议审查交叉验证）。
- [ ] **封装/解封装**：`flv/ts_mux/ts_demux/fmp4_mux/fmp4_demux/ps/rtp/jtt1078/mp4` 的 box/包上限
      （如 `max_box_bytes`、`max_reassembly_bytes`）是否都有硬上界（`AGENTS.md` §9）。
- [ ] **无协议状态机**：codec 不实现 RTMP/RTSP/WebRTC/SIP 协议状态机；无 FFmpeg 类型泄漏到公共接口。
- [ ] **兼容层集中**（`compat.rs`）：厂商 quirks 显式命名、集中管理，未打散到热路径。
- [ ] **codec 矩阵一致性**：README 声明的编解码矩阵（H264/H265/H266/VP8/VP9/AV1/MJPEG；AAC/G711/Opus/MP3/MP2）
...
- [ ] `RuntimeApi` 双通道任务模型：`spawn`（`Send` 主路径）+ `spawn_local`（browser/WASI），
      未为 wasm 削弱多线程主路径。
- [ ] 公共接口 runtime 中立；`tokio`/`tokio-util` 仅存在于 `cheetah-runtime-tokio`。
- [ ] 取消（`CancellationToken`）、任务句柄、完成通知等原语抽象完整，供 module 使用。
...
- [ ] module 契约、引擎注入能力定义清晰；不反向依赖具体协议 module（`AGENTS.md` §2）。
- [ ] HTTP module 契约框架无关（无 Axum/Tide/Actix 泄漏）。
- [ ] `EngineContext` / `StreamKey` / 发布租约 / 生命周期（`create -> init -> start`）契约完整。
- [ ] 宏（`sdk-macros`）生成代码不引入跨层依赖或 runtime 绑定。

### 3.2 `cheetah-config`

`src/lib.rs(582)`。审查项（对照 README §2）：

- [ ] 配置加载顺序：默认值 → `CHEETAH_CONFIG` YAML → `M7S_` 前缀环境变量。
- [ ] `M7S_GLOBAL__...` / `M7S_MODULE__<module>__...` 双下划线分层解析；bool/int/float/string 类型解析正确。
- [ ] 配置模型与 `config.example.yaml`、各 module 配置字段一致（端口、队列容量、超时、TLS 等）。
...
- [ ] **单发布者租约**：同一 `StreamKey` 默认单发布者独占语义，无绕过路径。
- [ ] **模块生命周期**：`ModuleRestartRequired` 由基础层执行 `create->init->start`；module 不自维护私有重启。
- [ ] `ModuleManagerApi::restart_module/restart_modules` 只接受 `Running`，否则返回 `Conflict`。
- [ ] **Dispatcher / RingBuffer / subscriber queue**：慢订阅者不拖累其他订阅者；所有队列有上界。
- [ ] 热路径无阻塞、无 contended mutex；`Arc<AVFrame>`/`Bytes`/有界缓冲使用得当。
- [ ] `core_adapters.rs` 是否把 core 的 Sans-I/O 输出正确桥接到引擎，无业务逻辑下沉到 core。

### 3.4 `cheetah-control`

`src/lib.rs(989)`。

- [ ] 控制面 API（REST）非阻塞；与各 module 暴露的 REST 端点风格统一（ZLM/SMS 兼容点）。
- [ ] 鉴权、路由、错误语义一致。
...
- [ ] 作为 system 层 module，遵守 module 约束（走 `EngineContext`，不直接依赖 tokio net/time/sync）。
- [ ] 录制走 codec 导出视图，不复制时间戳/NALU/参数集逻辑。
- [ ] `zlm_compat` 兼容逻辑集中、显式命名。
...
- [ ] 不依赖 tokio/任何 runtime；不持有 socket/listener/stream；不 spawn 线程/任务。
- [ ] 无 `async fn` 作为核心状态机接口；不调用 `Instant::now()`/系统时间。
- [ ] 不访问 DB/HTTP/`EngineContext`/`StreamManager`/`RoomService`；无业务编排。
- [ ] I/O 为显式 `Input / Output / Event / Timer` 模型。

**B. `driver-tokio`（`AGENTS.md` §5）**
- [ ] 收包/发包/分帧/组帧/timer/spawn/channel/backpressure 均在此层。
- [ ] TCP framing / UDP 收发在 driver，不在 core；不持有业务状态。
- [ ] runtime 抽象通过 `RuntimeApi` 注入；公共接口不泄漏 tokio 类型。

**C. `module`（`AGENTS.md` §6）**
- [ ] 通过 `EngineContext` 交互；不重写协议状态机；不直接用 `tokio::net/time/sync`、`tokio::select!`。
- [ ] 资源分配/会话绑定/鉴权/API 路由/业务映射在此层；单发布者租约不被绕过。
- [ ] 不复制 codec 的时间戳修复/NALU/参数集逻辑。

**D. 测试（`AGENTS.md` §11 + `SystemArchitecture.md` §6）**
- [ ] core：单测 + 属性测试 + fuzz（无真实网络 I/O）。
- [ ] driver：I/O 集成测试。
- [ ] module：互操作 + 端到端。
- [ ] 时间戳/重排/参数集补发/兼容修复均有回归测试。

**E. 一致性**
- [ ] 与 `SystemArchitecture.md` Reference Mapping、README 使用说明、`config.example.yaml` 一致。

### 4.1 RTP（`crates/protocols/rtp/`）
- [ ] Auto Probe：TS(`0x47`)/PS(`0x000001BA`)/Raw ES/Ehome/Hikvision XHB/JT1078 探测正确。
- [ ] Jitter buffer 有界；PS 重组 `max_reassembly_bytes`(4MB) 硬上限；空闲/超时清理句柄。
- [ ] UDP/TCP（RFC4571 2字节 + RTSP 4字节 interleaved 自动识别）；RTCP SR/RR、RR-timeout。
- [ ] 与 RTMP/RTSP/HLS/fMP4 的双向桥接（引擎内）正确。
- 命令：`cargo test -p cheetah-rtp-core/-driver-tokio/-module/-property-tests`

### 4.2 TS（`crates/protocols/ts/`）
- [ ] `TsCore` 仅处理 HTTP 头/CORS/WS 升级/会话取消；PAT/PMT 周期注入（`pat_pmt_interval_ms`）。
- [ ] GOP 缓存（`bootstrap_max_frames`）；`strict_crc`、`max_reassembly_bytes` 上界。
- [ ] RTP-TS ingest（`RtpTsIngest`）解复用正确；TLS(8445) 与 `.live.ts` 兼容 URL。
- [ ] **缺 property-tests** → 评估是否补齐（对齐其他协议基线）。
...
- [ ] HTTP-FLV / WS-FLV 路由 `/{app}/{stream}.flv`；pull job 重试 backoff。
- [ ] fixture 回放（`tests/testdata/http-flv/{standard,probes}`）与 transport fault 视图。
...
- [ ] `Fmp4Core` 纯状态机；init(ftyp+moov)/media(styp+moof+mdat) 走 codec `Fmp4Muxer`。
- [ ] `max_tracks/max_box_bytes/max_fragment_duration_ms` 上界；`.mp4`/`.live.mp4` 路由；TLS(8446)。
- [ ] bootstrap GOP 追赶；pull(http/https/ws/wss) backoff。
...
- [ ] 三段式与 codec `mp4/{writer,reader,sample_table}` 分工；点播/录制路径。
- [ ] **补 SystemArchitecture Reference Mapping**（当前缺）。
- 命令：`cargo test -p cheetah-mp4-core/-driver-tokio/-module/-property-tests`

### 4.6 HLS / LLHLS（`crates/protocols/hls/`）
- [ ] `ts`/`fmp4` container 切换；`ll_hls_enabled`、`part_target_ms`、`segment_*` 语义。
- [ ] LLHLS 标签（`SERVER-CONTROL/PART-INF/PART/PRELOAD-HINT/PROGRAM-DATE-TIME`）正确。
- [ ] Session（Cookie `HLS_SESSION` + `?session=`）；CDN `cdn_secret` Bearer 与缓存语义。
- [ ] **补 SystemArchitecture Reference Mapping**（当前缺）。
...
- [ ] core `#![no_std] + alloc`，无 std 特性开关即可 host/cross 编译；无连接门面（`RtmpServerConnection` 等）残留。
- [ ] runtime wiring 仅在 driver（`start_server`/`start_client`）；module 拥有 pull/push 后台任务编排。
- [ ] c-api / wasm 仅 core 绑定，不搬运 driver/module 逻辑、不改热路径。
- [ ] Enhanced-RTMP、`vendor-ref/simple-media-server` 对齐点（见 `dev-docs/RTMPAlignmentWithSimpleMediaServer.md`）。
...
- [ ] 控制面方法齐全（OPTIONS/DESCRIBE/ANNOUNCE/SETUP/PLAY/PAUSE/RECORD/TEARDOWN/GET_/SET_PARAMETER）。
- [ ] 传输矩阵：UDP unicast / TCP interleaved / HTTP tunnel(GET+POST+cookie+base64) / multicast PLAY。
- [ ] core 覆盖解析（Transport/Session/Range/RTP-Info/SDP/auth）+ interleaved framing + RTP/RTCP 模型。
- [ ] driver 拥有 HTTP tunnel 连接配对、multicast 端点、outbound client；module 拥有租约/会话/pull/push/relay 监管。
...
- [ ] `Gb28181Core` Sans-I/O：SIP(REGISTER/MESSAGE/INVITE/ACK/BYE)、设备注册表、keepalive 超时、SDP(`GbSdp`)。
- [ ] MD5 挑战鉴权；宽松行终止符（`\r\n`/`\n`/`\r`）与 `,`/`;` Digest 解析；重复头容忍。
- [ ] INVITE 分配 RTP 端口 + SSRC；BYE/离线闭环清理租约与端口映射，无句柄泄漏。
- [ ] REST 端点（Catalog/INVITE/BYE/talkback）ZLM/SMS 兼容。
- 命令：`cargo test -p cheetah-gb28181-core/-driver-tokio/-module/-property-tests`

### 4.10 SRT（`crates/protocols/srt/`）
- [ ] 三段式边界；依赖 `shiguredo_srt` 的封装是否局限在 driver/core 适配层。
- [ ] 握手/加密/延迟窗口/带宽估计等缓冲的上界。
- [ ] **补 SystemArchitecture Reference Mapping**（当前缺）；对照 `dev-docs/plans-28-srt`。
- 命令：`cargo test -p cheetah-srt-core/-driver-tokio/-module/-property-tests`

### 4.11 WebRTC（`crates/protocols/webrtc/`，规模最大）
- [ ] 基于 `str0m`（Sans-I/O）：确认 core 保持 Sans-I/O，`str0m` 类型不泄漏到 sdk/engine 公共接口。
- [ ] 信令（WHIP/WHEP / ZLM / OME 兼容，见 `tests/fixtures/{zlm,ome}`）、SDP 协商、ICE/DTLS/SRTP 边界。
- [ ] P2P 路径（`module/src/p2p`）职责；发布租约与引擎桥接。
- [ ] **补 SystemArchitecture Reference Mapping**（当前缺）。
...
- [ ] 运行报告暴露 `startup_latency_ms`、`first_second_avg_frame_interval_ms`、`average_playback_rate_x`、`first_keyframe_delay_ms`。
- [ ] 时间戳修复告警分层：`source_repair_events` / `canonical_repair_events` / `egress_repair_events`；
      正常 B 帧重排不升级 canonical/egress 告警；高频告警对阈值（`REPAIR_WARN_HIGH_FREQUENCY_THRESHOLD`）判定。
...
- [ ] 按 `AGENTS.md` §13 更新 `SystemArchitecture.md`（补 hls/ts/mp4/srt/webrtc 映射）、`AGENTS.md`、README、`config.example.yaml`。
- [ ] 归档审查报告：按 §0.2 格式汇总各阶段 `结论 + 证据 + 建议动作`，区分“违规（必须修）/风险（建议修）/观察（记录）”。
- [ ] 提交前最低检查（`AGENTS.md` §12）：`cargo fmt` → `cargo clippy -p <changed>` → `cargo test -p <changed>`；
