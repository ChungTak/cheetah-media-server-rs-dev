# 点播与多格式录制总体架构设计（对标 ZLMediaKit）

- **状态**: 已完成
- **范围**: 固定 MP4 VOD 与统一录制的 crate 边界、共享媒体 API、REST 控制模型、跨协议桥接方式和 ZLM 兼容策略
- **完成标准**: 实现者能够按本文拆出 `cheetah-mp4-*` 与 `cheetah-record-module`，并把容器、时间戳、seek 和录制任务逻辑放到正确层级

## 架构目标

本次能力分成两条主线：

1. **统一录制主线**：对 engine live stream 建立录制任务，按格式写出 `FLV/HLS/HLS-FMP4/MP4/PS/TS/FMP4`，首批验收 `FLV/HLS/MP4/PS`。
2. **MP4 VOD 主线**：把本地 MP4 文件或 MP4 目录读取成统一 `AVFrame + TrackInfo`，通过 engine 和现有协议模块对外点播，并支持 seek、pause、speed、stop。

首版实现策略：

- 录制和点播都复用 `AVFrame + TrackInfo`
- 容器读写、sample index、时间戳归一化和 codec 兼容尽量下沉到 `cheetah-codec`
...
- `cheetah-codec` 负责 classic MP4、FLV、PS、HLS/HLS-FMP4 segment 视图和时间戳模型，不负责 engine 任务编排
...
- MP4/FLV/PS/TS writer 都基于统一 canonical 时间线
- HLS writer 输出 `segment/init/playlist` 事件，不直接做磁盘 I/O
- H264/H265 参数集缓存、AAC ADTS/ASC、G711/MP3/Opus、VP8/VP9/AV1 视图复用现有 codec helper
- classic MP4 reader 提供 sample index、track map、seek、duration、多轨 frame 输出
...
RTSP/RTMP/HTTP-FLV/WS-FLV request
  ...
  -> TrackInfo + AVFrame
  -> Engine StreamManager / protocol bridge
```

规则：

- VOD session 以 `session_id` 标识，并绑定 source file、target stream key、loop_count
- 支持单文件、分号分隔多文件和目录内多 MP4 文件串联回放
- seek 后必须清空旧缓冲并从新位置的可用同步点恢复
- pause 只停止 pacing，不销毁 reader
- speed 首版支持 `0.1..=20.0`，默认 `1.0`
- RTSP `Range/Scale`、RTMP `seek/onPlayCtrl`、HTTP query `seek/speed` 最终统一映射到 `VodControlApi`
...
```text
./record/{format}/{vhost}/{app}/{stream}/{yyyy-mm-dd}/{yyyy-mm-dd-hh-mm-ss-index}.{ext}
```
...
- RTMP `mp4:` stream id 还原为 `.mp4` 文件名
