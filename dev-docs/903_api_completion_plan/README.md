...
- [x] 能力、operation、URL 与 module/provider 实际状态一致。
- [x] RTP 收发、参数更新、超时和清理有真实网络测试。
- [x] JPEG 可解码；文件删除同时清理受控物理文件和元数据。
- [x] MP4 回放实际读取、解复用并向引擎输出媒体帧。
- [x] RTSP 拉流、RTMP 推流和 FFmpeg 任务执行有成功链路。
- [x] 准入决策进入发布/播放主路径，事件可由外部服务可靠订阅。
- [x] 资源级授权、deadline、幂等和 SSRF 防护有负向测试。
- [x] 四类信令合同同时覆盖 Rust SDK 与独立 HTTP 客户端视角。
- [x] 发布报告中的每个结论都有命令、日志或制品证据。
