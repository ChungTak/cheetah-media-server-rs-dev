...
#[tokio::test]
async fn runtime_http_client_rejects_oversized_response() {
    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let addr = listener.local_addr().unwrap();
    ...
    tokio::spawn(async move {
        ...
        let mut buf = [0u8; 1024];
        let n = stream.read(&mut buf).await.unwrap();
        let _received = String::from_utf8_lossy(&buf[..n]);
        // Claim a 100-byte body but the client limits response to 10 bytes.
        let response = b"HTTP/1.1 200 OK\r\nContent-Length: 100\r\n\r
over-sized response body that exceeds the configured response limit";
        stream.write_all(response).await.unwrap();
    });
