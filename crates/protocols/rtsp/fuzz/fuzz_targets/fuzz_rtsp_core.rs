...
fuzz_target!(|data: &[u8]| {
    ...
    common::fuzz_core_entry(&bounded, limits.clone(), 7);
    common::fuzz_message_decoders(&bounded, limits.clone(), 5);

    let mixed =
        common::build_mixed_rtsp_interleaved_input(&bounded, limits.max_interleaved_frame_size);
    common::fuzz_core_entry(&mixed, limits.clone(), 9);
    common::fuzz_message_decoders(&mixed, limits, 11);
});
