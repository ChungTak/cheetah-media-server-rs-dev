...
//! * Valid `rtc://host/app/stream` inputs round-trip the host, app and
//!   stream segments verbatim (no implicit normalisation that the
//!   caller cannot observe).
//! * Re-parsing a syntactically-valid URL is idempotent at the field
//!   level.
//!
//! The proptest strategy generates only the inputs we want to assert
//! over (printable ASCII path segments, ASCII hosts, optional ports);
//! a separate fuzz target covers the panic-free invariant for
//! arbitrary bytes.
...
fn arb_path_segment() -> impl Strategy<Value = String> {
    // Use a tight subset of URL-safe characters so we do not have to
    // model percent-encoding for the round-trip assertion.
    "[a-zA-Z0-9_\\-]{1,16}"
}
...
proptest! {
    /// Parsing must never panic, even on arbitrary text. The parser
    /// returns `Result` so any input either yields a structured URL
    ...
    /// Well-formed URLs round-trip the host, app and stream segments.
    /// We do not yet have a `to_string()` on `ZlmRtcUrl`, so the
    /// round-trip is field-level: we re-render the canonical form
    /// and re-parse, asserting equality on the parsed structure.
    #[test]
    fn well_formed_rtc_url_roundtrips_fields(
        ...
        // Re-parse the rendered canonical form. Idempotency at the
        // field level guards against any latent state in the parser
        // (interner, regex side effects, etc.).
    ...
    /// `signaling_protocols` is always parsed as an unsigned integer
    /// when present and well-formed; passing the literal `0` or `1`
    /// must yield those numeric values.
    #[test]
    ...
    /// Unknown query parameters are surfaced verbatim through
    /// `extra_params`, preserving their order. ZLM-style clients rely
    /// on this for opaque keys like `secret=...`.
    #[test]
    fn extra_query_params_preserve_keys(
        ...
        let saw_keys: std::collections::HashSet<&str> = parsed
            ...
            .collect();
        for k in &keys {
            // Keys are always preserved (some duplicates may collapse
            // when the generator picks the same string twice; we use
            // a set membership check instead of equality).
