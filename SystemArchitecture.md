...
- `EngineMediaFacade` invokes admission before `acquire_publisher`, `open_subscriber`, `create_pull_proxy`, `create_push_proxy`, `open_rtp_receiver` and `open_rtp_sender`. A `Deny` returns the stable `MediaErrorCode` before the provider allocates any lease, port or session.
